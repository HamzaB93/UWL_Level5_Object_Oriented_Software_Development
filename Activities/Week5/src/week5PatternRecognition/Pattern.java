package week5PatternRecognition;
			...
			for (int i = 0; i < numbers.length; i++)
				for (int j = 0; j < numbers[i].length; j++) {
					...
					if (tfValues[i][j].getText().trim().length() == 0) {
						...
						numbers[i][j] =
						Integer.parseInt(tfValues[i][j].getText().trim());
				}

			int[][] result = isConsecutiveFour(numbers);
			if (result != null) {
				lblStatus.setText("A consecutive four found");
				tfValues[result[0][0]][result[0][1]].setStyle("-fxbackground: red");
						tfValues[result[1][0]][result[1][1]].setStyle("-fxbackground: red");
								tfValues[result[2][0]][result[2][1]].setStyle("-fxbackground: red");
										tfValues[result[3][0]][result[3][1]].setStyle("-fxbackground: red");
			}
			else {
